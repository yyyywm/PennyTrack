...
- **每日记账提醒**：基于系统 AlarmManager 的本地通知调度，可设置多个时间点，点击通知直接弹出记账面板，**与登录态无关**，不需要应用常驻后台

## 测试环境

| 项目 | 测试设备 / 版本 |
|---|---|
| 主测设备 | **OnePlus 15（一加 15）** |
| 系统 | **ColorOS 16**（基于 Android 15） |
| Flutter | 3.19+ |
| 后端运行环境 | Ubuntu 22.04 + Docker 24.0 + MySQL 8.0 |

> 本应用**主要在 ColorOS 16 真机上完成开发与测试**。其他原生 Android（10+）以及主流国产 ROM（MIUI / OriginOS / EMUI）的核心功能可正常运行，但每日提醒在国产 ROM 上需要手动配置「自启动 / 后台运行 / 电池优化白名单」，详见下文「每日提醒使用说明」。
...
- **flutter_local_notifications + timezone** — 每日记账提醒，调度走系统 AlarmManager
- **permission_handler / device_info_plus / android_intent_plus** — 通知权限申请与国产 ROM 自启动 / 电池白名单深链跳转
...
- 在「我的」页面 → 「退出登录」按键下方，可看到 **每日记账提醒** 卡片
- 支持开启/关闭、添加多个时间点、点击时间块即可修改
- 点击通知会**直接弹出记账面板**（无需手动打开应用）
- **不依赖登录**，离线状态下也可使用
- 调度由系统 `AlarmManager` 接管，**无需应用常驻后台**，几乎不耗电

### 技术实现

- `flutter_local_notifications` + `AndroidScheduleMode.exactAllowWhileIdle`
- `DateTimeComponents.time` 实现每日同时间重复
- 应用启动时通过 `NotificationService.rescheduleFromStorage()` 兜底重注册，
  规避国产 ROM「锁屏清理」杀死定时任务的情况
- `RECEIVE_BOOT_COMPLETED` + `ScheduledNotificationBootReceiver` 处理设备重启

### ColorOS / 国产 ROM 配置（重要）

ColorOS 16、MIUI、HyperOS、OriginOS 等会主动冻结后台 App 的定时任务。
即便我们使用了系统级精确闹钟，仍可能被「电池管家」二次拦截。
**首次开启提醒时，请按提示完成下面三步配置：**

1. **自启动管理**：将 PennyTrack 加入白名单
   - ColorOS 路径：设置 → 应用管理 → PennyTrack → 自启动 → 开启
2. **电池优化**：将 PennyTrack 设为「不优化」
   - 提醒卡片中点击「电池优化」按钮，将 PennyTrack 加入「不限制」名单
3. **通知权限**：确保通知和锁屏通知都已开启
   - 提醒卡片中点击「通知设置」按钮，开启所有通知开关

> 国产 ROM 的「自启动管理」入口在不同 OEM / 不同版本上路径不同，
> 应用内提供的快捷按钮会自动尝试 ColorOS / MIUI / EMUI / Funtouch 等已知 Activity，
> 如果无法直接打开，会回落到系统的「应用详情」页，你可以手动找到对应开关。

### 兼容性说明

| 系统 | 支持情况 |
|---|---|
| 原生 Android 12 - 15 | ✅ 完全支持（标准 AOSP AlarmManager） |
| ColorOS 13 / 14 / 15 / 16 | ✅ 已测试，需配置自启动 + 电池白名单 |
| MIUI / HyperOS | ✅ 兼容，需手动加入「自启动」白名单 |
| EMUI / HarmonyOS（双框架） | ⚠️ 兼容，部分版本可能延迟数分钟 |
| OriginOS | ⚠️ 兼容，需关闭「省电模式」 |
...
1. 是否开启了「我的 → 每日记账提醒」总开关？
2. 通知权限是否被系统拒绝？卡片中点击「通知设置」打开系统页确认
3. **国产 ROM 必查**：是否将本应用加入「自启动管理」白名单？
4. **国产 ROM 必查**：是否将本应用设为「不优化电池」？
5. 是否打开了「省电模式」/「超级省电」？这两类模式会冻结所有定时任务
6. 测试时建议先点「发送一条测试通知」按钮，立刻验证通知通道是否正常
